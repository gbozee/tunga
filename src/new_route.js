...
const NestedComponent = ({ children, path, parent, ...propsRest }) => {
  return (
    <Switch>
      {children.map((child, index) => {
        const new_route = generateUrl(child.path, path);
          ...
          return (
            <RenderRouteComponent
              ...
              render={({ match: { params }, location: { pathname } }) => {
                let uuu = generateUrl(child.to, path, params);
...
const TungeRoute = ({ base_url = "", ...rest }) => (
  <App
    base_path={generateUrl("", base_url)}
    replaceFunc={router => {
      const replacer = router.history.replace;
      return replacer;
    }}
    oldComponentDidMountAction={(obj, value) => {
      const obj2 = queryString.parse(obj.search);
      return obj2[value];
    }}
    {...rest}
  >
...
export default ({ listeningFunction }) => {
  ...
  return (
    <Router>
      <App20 routes={unauthorized_urls} func={listeningFunction} />
    </Router>
  );
};

const App21 = ({ func, ...rest }) => {
  rest.history.listen(func);
  return (
    <Switch>
      <Route
        path="/tunga"
        render={() => <TungeRoute base_url="/tunga" {...rest} />}
      />
      <Route path="/" render={() => <TungeRoute {...rest} />} />
    </Switch>
  );
};
const App20 = withRouter(App21);
